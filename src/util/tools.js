/**
 * Created by jenasli on 2019/10/16.
 */
/* eslint-disable */
export default {
  getDefaultFormatDate : function (date) {
  ...
  getFormatDateWithTime : function (date, fmt) {
  ...
  exportCsv: function (headers, rows, filename) {
    ...
    if (Array.isArray(headers) && headers.length > 0) { //表头信息不能为空
      ...
      let blob = _this.getCsvBlob(headers, rows);
  ...
  getCsvBlob : function (headers, rows) {
  const BOM = '\uFEFF';
